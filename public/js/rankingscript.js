...
$(document).ready(function() {
		$('#printRanking').click(function() {
			identifier++;
			$('.frmb').append(
				"<div class='ranking-container-"+identifier+"'>"+
						"<div class='container-buttons' align='right'>"+
                "<h6>Ranking</h6>"+
						    "<button class='btn' style='color:white;' id='button-addQues-"+identifier+"' onclick='addQues("+identifier+")'>Agregar pregunta</button>"+
                ...
                "<hr />"+
						"</div>"+
						"<div class='ranking-content-"+identifier+"'></div>"
				+"</div>"
		    );
  ...
  function addQues(identifier) {
  	let inputIdentifier = Math.floor(Math.random() * 10000000 + 1);
 	 		    $(".ranking-content-"+identifier+"").append(
             ...
             "<label for='question'>Desarrolle la pregunta:</label><br>"+
                ...
                "<hr />"+
             "<div align='right'>"+
					   "<button class='btn' style='color:white;' onclick='addItem("+identifier+")'><i class='fa fa-plus-square'></i></button>"+
					   "<button class='btn btn-danger' style='color:white;' btn-danger' onclick='removeInput("+inputIdentifier+")'><i class='fa fa-window-close'></i></button>"+
             "</div>"+
				 "<div>"	   
 	 		    );
   }
  ...
  function addItem(identifier) {
    let inputIdentifier = Math.floor(Math.random() * 10000000 + 1);
          $(".ranking-content-"+identifier+"").append(
           "<div class='input-component-"+inputIdentifier+"'>"+ 
              ...
              "<input class='form-control' id='item' type='text'><br>"+
             "<div align='right'>"+
             "<button class='btn' style='color:white;' onclick='addItem("+identifier+")'><i class='fa fa-plus-square'></i></button>"+
             "<button class='btn btn-danger' style='color:white;' btn-danger' onclick='removeInput("+inputIdentifier+")'><i class='fa fa-window-close'></i></button>"+
             "</div>"+
   ...
   function generateMatrix(identifier) {
   		...
   		$("#button-addQues-"+identifier+"").hide(400);
